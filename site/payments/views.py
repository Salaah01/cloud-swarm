...
@csrf_exempt
def handle_subscription_callback(request: http.HttpRequest):
    WEBHOOK_SECRET = 'whsec_LsVrgnSUkBbYTYKsYF4rrpSinxbPvzyp'
    request_data = json.loads(request.body)

    # Verify the webhook event by checking the signature
...
class SamplePayment(View):
    def get(self, request):
        return render(request, 'payments/sample-payment.html')
